# App Store listing copy — Driver SK

Final store metadata for App Store Connect, three locales (EN / SK / HU).
Character limits: Promotional text ≤ 170 · Keywords ≤ 100 · Description ≤ 4000.
Subtitle ≤ 30 (not yet written — see note at the bottom).

Shared URLs:
- Privacy Policy: https://www.smartie.team/driver-sk/privacy
- Terms of Use: https://www.smartie.team/driver-sk/terms
- Support: https://www.smartie.team/driver-sk/support
- Support email: hello-driver@smartie.team

> Keep the "Driver SK Pro" tier list in the description in sync with what's
> actually purchasable. Only keep the "3-day free trial" lines if the trial is
> live on the yearly product.

---

## English (US)

### Promotional text (138)
Full-length mock exams and smart practice that targets exactly what trips you up, then drills it until it sticks. Try Pro free for 3 days.

### Keywords (100)
driving,theory,licence,license,exam,test,quiz,practice,mock,signs,traffic,rules,road,permit,Slovakia

### Description (2631)
Pass the Slovak driving theory test with focused, modern practice — in Slovak, English, or Hungarian.

Driver SK turns the official question bank into a study plan that adapts to you. Instead of grinding through random questions, you practice exactly what you keep getting wrong, take realistic mock exams, and watch your readiness climb day by day.

SMART STUDY
Adaptive practice that prioritizes your weak spots. It surfaces the questions you miss, the ones you answer slowly, and the categories you haven't mastered — so every minute counts.

REALISTIC MOCK EXAMS
Full-length practice tests that mirror the real exam: the same number of questions, the same time limit, the same scoring. Know exactly where you stand before test day.

MASTER YOUR MISTAKES
Every wrong answer goes into a dedicated review deck with spaced repetition, so a slip becomes knowledge that sticks — not the same mistake on exam day.

TRACK YOUR READINESS
A clear readiness score, accuracy trends, daily streaks, and category breakdowns show your progress at a glance and keep you motivated.

LEARN RIGHT OF WAY BY DRIVING
Drive through endless Slovak junctions in a top-down game where every other car follows the real priority rules: signs, traffic lights, the right-hand rule, trams and roundabouts. A step-by-step guide first teaches the controls and every type of junction, and the drive log explains each decision afterwards.

STUDY ANYWHERE
Fully offline. The entire question bank — including road-sign images — works with no connection: on the bus, in a queue, anywhere.

THREE LANGUAGES
The complete question bank in Slovak, English, and Hungarian. Perfect if you're taking the Slovak test but think in another language. Switch anytime.

DAILY REMINDERS
Gentle, optional reminders help you keep a streak and stay consistent — the single biggest predictor of passing.

Whether you're starting from zero or doing a final review the night before, Driver SK helps you walk in prepared and confident.

— Driver SK Pro —
Unlock unlimited Smart Study and Mistakes review:
• Weekly or Yearly, each with a 3-day free trial
• Lifetime — pay once, keep access forever

Have a promo code? Redeem it in Settings or on the Pro screen.

Payment is charged to your Apple ID at confirmation of purchase. Subscriptions renew automatically unless canceled at least 24 hours before the end of the current period. Manage or cancel anytime in your App Store account settings. Any unused portion of a free trial is forfeited when you buy a subscription.

Privacy Policy: https://www.smartie.team/driver-sk/privacy
Terms of Use: https://www.smartie.team/driver-sk/terms

---

## Slovak

### Promotional text (134)
Plné skúšobné testy a inteligentné štúdium presne na tvoje slabiny — a opakovanie, kým to nemáš v malíčku. Vyskúšaj Pro 3 dni zadarmo.

### Keywords (99)
vodičák,autoškola,vodičský,preukaz,testy,skúška,teória,značky,doprava,otázky,kvíz,pravidlá,príprava

### Description (2599)
Priprav sa na skúšku z autoškoly modernou a cielenou prípravou — v slovenčine, angličtine alebo maďarčine.

Driver SK premení oficiálnu databázu otázok na študijný plán, ktorý sa prispôsobí tebe. Namiesto náhodných otázok precvičuješ presne to, čo ti nejde, píšeš realistické skúšobné testy a sleduješ, ako tvoja pripravenosť každý deň rastie.

INTELIGENTNÉ ŠTÚDIUM
Adaptívne precvičovanie zamerané na tvoje slabiny. Vyberá otázky, ktoré chybuješ, na ktoré odpovedáš pomaly, aj okruhy, ktoré ešte nemáš zvládnuté — aby sa každá minúta rátala.

REALISTICKÉ SKÚŠOBNÉ TESTY
Plnohodnotné testy ako ostrá skúška: rovnaký počet otázok, rovnaký časový limit, rovnaké bodovanie. Presne vieš, ako na tom si, ešte pred skúškou.

ZVLÁDNI SVOJE CHYBY
Každá nesprávna odpoveď ide do samostatného opakovania rozloženého v čase, takže z chyby sa stane vedomosť, ktorá ti zostane — a nie rovnaké zaváhanie na skúške.

SLEDUJ SVOJU PRIPRAVENOSŤ
Jasné skóre pripravenosti, vývoj presnosti, denné série a rozpis podľa okruhov ti na prvý pohľad ukážu pokrok a udržia ťa motivovaným.

PREDNOSŤ V JAZDE ZA VOLANTOM
Prejdi nekonečné slovenské križovatky v hre, kde každé auto dodržiava skutočné pravidlá prednosti: značky, semafory, pravidlo pravej ruky, električky aj kruhové objazdy. Sprievodca ťa najprv krok po kroku naučí ovládanie a každý typ križovatky a denník jázd potom vysvetlí každé rozhodnutie.

ŠTUDUJ KDEKOĽVEK
Úplne offline. Celá databáza otázok — vrátane obrázkov dopravných značiek — funguje aj bez pripojenia: v autobuse, v rade, kdekoľvek.

TRI JAZYKY
Kompletná databáza otázok v slovenčine, angličtine a maďarčine. Ideálne, ak robíš slovenskú skúšku, ale rozmýšľaš v inom jazyku. Prepínaj kedykoľvek.

DENNÉ PRIPOMIENKY
Jemné, voliteľné pripomienky ti pomôžu udržať sériu a pravidelnosť — najsilnejší predpoklad úspechu.

Či začínaš od nuly, alebo si robíš poslednú prípravu večer pred skúškou, Driver SK ti pomôže prísť pripravený a sebavedomý.

— Driver SK Pro —
Odomkni neobmedzené Inteligentné štúdium a opakovanie chýb:
• Týždenne alebo ročne, vždy s 3-dňovou skúškou zadarmo
• Doživotne — zaplať raz a maj prístup navždy

Máš promo kód? Uplatni ho v Nastaveniach alebo na obrazovke Pro.

Platba sa strhne z tvojho Apple ID pri potvrdení nákupu. Predplatné sa automaticky obnovuje, ak ho nezrušíš aspoň 24 hodín pred koncom obdobia. Spravovať alebo zrušiť ho môžeš kedykoľvek v nastaveniach Apple ID. Nevyužitá časť skúšobného obdobia zaniká pri kúpe predplatného.

Zásady ochrany súkromia: https://www.smartie.team/driver-sk/privacy
Podmienky používania: https://www.smartie.team/driver-sk/terms

---

## Hungarian

### Promotional text (134)
Teljes próbavizsgák és okos gyakorlás pontosan a gyenge pontjaidra, majd ismétlés, amíg végleg megy. Próbáld ki a Prót 3 napig ingyen.

### Keywords (100)
jogosítvány,jogsi,szlovák,vezetés,KRESZ,vizsga,tesztek,elmélet,táblák,közlekedés,szabályok,gyakorlás

### Description (2673)
Készülj a szlovák KRESZ-vizsgára modern, célzott gyakorlással — szlovákul, angolul vagy magyarul.

A Driver SK a hivatalos kérdésbankot személyre szabott tanulási tervvé alakítja. Véletlenszerű kérdések helyett pontosan azt gyakorlod, ami nem megy, valódi próbavizsgákat írsz, és napról napra látod, ahogy nő a felkészültséged.

OKOS TANULÁS
Adaptív gyakorlás, amely a gyenge pontjaidra összpontosít. Előveszi a kérdéseket, amelyeket elrontasz, amelyekre lassan válaszolsz, és a kategóriákat, amelyeket még nem sajátítottál el — hogy minden perc számítson.

VALÓSÁGHŰ PRÓBAVIZSGÁK
Teljes hosszúságú tesztek, mint az éles vizsga: ugyanannyi kérdés, ugyanannyi idő, ugyanaz a pontozás. Pontosan tudod, hol tartasz, még a vizsga előtt.

JAVÍTSD KI A HIBÁIDAT
Minden rossz válasz külön ismétlő paklimba kerül, időzített ismétléssel, így a hibából megmaradó tudás lesz — nem ugyanaz a baki a vizsgán.

KÖVESD A FELKÉSZÜLTSÉGED
Egyértelmű felkészültségi pont, pontossági trendek, napi sorozatok és kategóriánkénti bontás egy pillantásra megmutatják a fejlődésed, és motiválnak.

ELSŐBBSÉG VEZETÉS KÖZBEN
Vezess végtelen szlovák kereszteződéseken egy játékban, ahol minden autó a valódi elsőbbségi szabályokat követi: táblák, jelzőlámpák, jobbkéz-szabály, villamosok és körforgalmak. Egy lépésről lépésre haladó útmutató előbb megtanítja az irányítást és minden kereszteződéstípust, a menetnapló pedig utólag elmagyaráz minden döntést.

TANULJ BÁRHOL
Teljesen offline. A teljes kérdésbank — a közlekedési táblák képeivel együtt — internet nélkül is működik: buszon, sorban állva, bárhol.

HÁROM NYELV
A teljes kérdésbank szlovákul, angolul és magyarul. Ideális, ha a szlovák vizsgát teszed le, de más nyelven gondolkodsz. Válts bármikor.

NAPI EMLÉKEZTETŐK
Finom, opcionális emlékeztetők segítenek tartani a sorozatot és a rendszerességet — a siker legfőbb záloga.

Akár nulláról kezded, akár az utolsó ismétlést végzed a vizsga előtti este, a Driver SK segít felkészülten és magabiztosan érkezni.

— Driver SK Pro —
Oldd fel a korlátlan Okos tanulást és hibajavítást:
• Hetente vagy évente, mindkettő 3 napos ingyenes próbával
• Örökre — fizess egyszer, és örökre a tiéd

Van promóciós kódod? Váltsd be a Beállításokban vagy a Pro képernyőn.

A fizetés az Apple ID-dről történik a vásárlás megerősítésekor. Az előfizetés automatikusan megújul, hacsak nem mondod le legalább 24 órával az időszak vége előtt. Bármikor kezelheted vagy lemondhatod az Apple ID beállításaiban. Az ingyenes próba fel nem használt része elvész előfizetés vásárlásakor.

Adatvédelmi irányelvek: https://www.smartie.team/driver-sk/privacy
Felhasználási feltételek: https://www.smartie.team/driver-sk/terms

---

## TODO: Subtitle (≤ 30 chars, per locale)

Not yet written. The subtitle is indexed for search, so once chosen, de-dupe
it against the keyword lists above and reclaim any freed characters.
Draft ideas (verify length):
- EN: "Slovak driving test prep"
- SK: "Príprava na vodičák"
- HU: "Szlovák KRESZ-felkészítő"
