...
**Two sources of schema truth.** `migrate.ts` is the one that runs. `drizzle/migrations/` holds three files generated by `drizzle-kit` long ago that nothing executes and that stop before `exam_results`, `game_rounds` and `crossing_log`; treat them as history. If you change a table shape, update **both** the Drizzle schema file *and* `migrate.ts`; they drift silently otherwise (see `docs/sqlite-schema.md` for the differences that exist today).
...
- **`tsc --noEmit` is noisy and is not a gate.** It reports about 180 errors, most of them untyped `useState(null)` and implicit `any` in `app/stats.tsx`, `app/mock.tsx` and `app/mistakes.tsx`, plus expo-router's generated route union lagging behind new screens. Builds work regardless, because Babel strips types. When you must add a new prop, add it to the primitive's destructure and pass it through.
