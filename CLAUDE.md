...
The real game: an endless drive through generated junctions where every other vehicle obeys the Slovak priority rules. **`docs/game/runner.md` is the reference** for the runner (world model, scheduling, motion, penalties, events, rendering, debugging); this section is orientation only.

- `engine.js` resolves priority for a scene: police and lights, entries from non-roads, signs, the right-hand rule, left turns, trams, roundabouts, pedestrians, emergency vehicles. Every rule is cited in `docs/game/priority-rules.md` (zákon 8/2009, vyhláška 30/2020), whose last section lists where the runner deliberately differs from the exam.
- `geometry.js` decides whether two movements interact (five conflict cells plus the exam's conventions); `conflict.js` decides when a vehicle is physically out of your way; `layout.js` is the top-down coordinate system shared by the paths and the SVG layers; `timeline.js` moves the other vehicles; `queue.js` spaces vehicles sharing an arm; `generator.js` makes seeded scenes by level; `world.js` is the run itself.
- `data/game/scenes.json` holds all 39 exam pictures as scenes (format in `docs/game/scene-format.md`). **`__tests__/scenes.test.js` fails if the engine stops reproducing an official answer, so change the engine only with that suite green.**
- Controls are swipes only: down to give way, up to move off, left or right to turn. The car never moves off by itself. Teaching lives in the guide, not in the game.
- Every junction driven is recorded (`junctionRecord`) into `crossing_log` and explained in the drive log (`app/crossing-log.tsx`, `src/lib/crossingLog.js`); rounds go to `game_rounds` with `mode = 'crossing'`.
- Dev aid: `driver://crossing?seed=N&level=M` replays a known road in a Debug build (iOS asks "Open in Driver SK?" first) and Metro logs `[crossing] run seed=… level=…`.

### Guide mode (`app/crossing-guide.tsx`, `src/lib/priority/lessons.js`)
...
- **Maestro E2E** in `.maestro/` — flows `01_smoke`, `02_onboarding`, `03_navigation`, `04_study`, `05_settings`, `06_exam`, `07_game`, `08_crossing`, `09_guide`, running on iOS simulators and Android emulators. **Selector convention:** every interactive element has a `testID` equal to the **i18n key** of its label (e.g. `testID="home.smartStudyCta"`, `testID="settings.notificationsMorning"`), and every `<Screen>` carries `testID="screen.<name>"` for "are we here" assertions. This makes flows language-independent. The `testID` prop is threaded through the shared UI primitives in `components/ui/`. Re-recordings are committed at `.maestro/recordings/`.
