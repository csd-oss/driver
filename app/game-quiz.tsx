...
export default function GameQuizScreen() {
  ...
  const loadStats = useCallback(async (currentLang: number) => {
    const stats = await GameRoundsDB.getGameStats(currentLang, 'quiz');
    setBest(stats.best);
  ...
  useFocusEffect(
    useCallback(() => {
      trackScreenView(posthog, 'GameQuiz');
      getLanguage().then((l) => {
        setLang(l);
        loadStats(l);
      });
  ...
  return (
    <Screen testID="screen.gameQuiz" header={<Header title={t('crossing.modeQuiz', lang)} onBackPress={handleBack} />}>
